...
Resonance is a self-hosted, shared music player for a Plex Music library. This
policy describes the software's data handling. The person or organisation that
operates a Resonance server is responsible for its deployment, access controls,
backups, and any additional legal notices required for its users.

## Where data goes

The Android app and web client communicate only with the Resonance server URL
that the user chooses. Resonance has no central service operated by this
project, and it does not include advertising, analytics, telemetry, or a
third-party crash-reporting SDK.

To sign in and use a library, the configured Resonance server communicates with
Plex. Plex's own privacy policy and terms govern Plex's handling of data.
Resonance uses the Plex PIN/forwarding sign-in flow and sends Plex requests only
...
## Data stored by a Resonance server
...
Resonance server to web or Android clients. Server operators should protect the
...
The Android app stores the selected Resonance server URL and its Resonance
...
Resonance does not sell or share personal data with advertisers or analytics
providers. Data is shared with Plex only as described above and is visible to
the operator of the self-hosted Resonance server. Data remains in the local
...
Resonance server, contact that server's operator. The operator can remove the
...
Resonance is not affiliated with, endorsed by, or sponsored by Plex. Plex is a
