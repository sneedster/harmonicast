...
import { browseLibraryArtist, discoverLibrary, getMusicSourceExtension, launchMusicSourceExtension, searchLibrary, type DiscoveryShelf, type MusicSourceExtensionStatus } from '@/lib/api';
...
export function KioskView({ onExit }: { onExit: () => void }) {
  ...
  return <div className="h-[100dvh] overflow-hidden bg-[radial-gradient(circle_at_50%_-20%,_rgba(245,158,11,0.28),_transparent_42%),linear-gradient(135deg,_#07070b,_#18101c_52%,_#07070b)] text-ink-100" onPointerDownCapture={wakeKiosk} onKeyDownCapture={wakeKiosk}>
    ...
    <div className="h-[calc(100dvh-8rem)] overflow-hidden sm:h-[calc(100dvh-10rem)]" onTouchStart={(e) => { touchStart.current = e.touches[0]?.clientX ?? null; }} onTouchEnd={(e) => handleSwipe(e.changedTouches[0]?.clientX ?? 0)}><div className="flex h-full transition-transform duration-500 ease-out" style={{ width: `${pages.length * 100}%`, transform: `translateX(-${panelIndex * (100 / pages.length)}%)` }}>
      ...
      <Panel><Title eyebrow="Any song you want" title="Find music" /><div className="mt-5 flex h-16 gap-3"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Artist, song, or album" className="min-w-0 flex-1 rounded-2xl border border-ink-600 bg-ink-900 px-5 text-xl text-white outline-none focus:border-amber-400" /><div className="grid w-16 place-items-center rounded-2xl bg-white text-ink-950">{searching ? <Loader2 className="h-5 w-5 animate-spin" /> : <Search className="h-5 w-5" />}</div></div>{artistBrowse ? <ArtistBrowser artist={artistBrowse} onPick={requestSong} /> : <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">{results.slice(0, 8).map((song) => <Tile key={song.id} song={song} onPick={requestSong} />)}</div>}{query.trim() && !searching && musicSourceExtension && (!results.length || artistBrowse) && <div className="mx-auto mt-6 max-w-xl rounded-2xl border border-amber-400/30 bg-amber-400/10 p-6 text-center"><p className="text-lg font-black text-white">{artistBrowse ? 'Want to browse sources beyond Plex?' : 'Not in this library'}</p><p className="mt-2 text-sm text-ink-300">{artistBrowse ? `Browse songs by ${artistBrowse.name} in connected music sources.` : 'Search connected music sources for this song.'}</p><div className="mt-5 flex flex-wrap justify-center gap-3">{!results.length && <button onClick={() => void searchConnectedSources()} disabled={openingMusicSource} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-amber-400 px-5 font-black text-ink-950 disabled:opacity-60">{openingMusicSource && <Loader2 className="h-4 w-4 animate-spin" />} Search connected music sources</button>}{artistBrowse && <button onClick={() => void searchConnectedSources('artist', artistBrowse.name)} disabled={openingMusicSource} className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-amber-400 px-5 font-black text-ink-950 disabled:opacity-60">{openingMusicSource && <Loader2 className="h-4 w-4 animate-spin" />} Browse this artist</button>}</div></div>}{!results.length && !searching && !musicSourceExtension && <p className="mt-12 text-center text-xl text-ink-500">{query.trim() ? 'No matching tracks in this library.' : 'Start typing to search every track. Browse stays intentionally curated.'}</p>}</Panel>
...
function ArtistBrowser({ artist, onPick }: { artist: { name: string; songs: Song[] }; onPick: (song: Song) => Promise<void> }) {
