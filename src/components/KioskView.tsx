...
export function KioskView({ onExit }: { onExit: () => void }) {
  ...
  async function searchConnectedSources(mode: 'search' | 'artist' = 'search', artist = query.trim()) {
    ...
    try {
      const { launchUrl } = await launchMusicSourceExtension(musicSourceExtension.id, artist, mode);
  ...
  const normalizedQuery = query.normalize('NFKD').toLowerCase().replace(/[^a-z0-9]+/g, '');
  const sourceArtist = results.find((song) => song.artist.normalize('NFKD').toLowerCase().replace(/[^a-z0-9]+/g, '') === normalizedQuery)?.artist;
