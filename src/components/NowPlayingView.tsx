import { ThumbsDown, ThumbsUp, Volume2, Loader2, Radio, WandSparkles, Sparkles, X } from 'lucide-react';
import { useState } from 'react';
import { usePlayer } from '@/hooks/usePlayer';
import { CoverArt } from '@/components/CoverArt';
import { AndroidAppDownload } from '@/components/AndroidAppDownload';
import { formatTime } from '@/lib/format';
import { getPlexArtistDiscovery, type PlexArtistDiscovery } from '@/lib/api';

export function NowPlayingView({ onSearch }: { onSearch: (query: string) => void }) {
  const {
    isHost,
    isHostUser,
    current,
    plexRating,
    isPlaying,
    currentTime,
    duration,
    thumbsUp,
    thumbsDown,
    queueSimilar,
  } = usePlayer();
  const [discovery, setDiscovery] = useState<PlexArtistDiscovery | null>(null);
  const [discoveryLoading, setDiscoveryLoading] = useState(false);
  const [discoveryError, setDiscoveryError] = useState<string | null>(null);

  async function openDiscovery() {
    if (!current) return;
    setDiscoveryLoading(true); setDiscoveryError(null);
    try { setDiscovery(await getPlexArtistDiscovery(current.id)); }
    catch (error) { setDiscoveryError(error instanceof Error ? error.message : 'Could not load artist discovery.'); }
    finally { setDiscoveryLoading(false); }
  }

  const total = duration || current?.duration || 0;
  const pct = total > 0 ? Math.min(100, (currentTime / total) * 100) : 0;

  if (!current) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-ink-500 animate-fade-in">
        <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-ink-800">
          {isPlaying ? (
            <Loader2 className="h-8 w-8 animate-spin" />
          ) : (
            <Radio className="h-8 w-8" />
          )}
        </div>
        <p className="text-sm font-medium text-ink-300">Nothing playing right now</p>
        <p className="text-xs text-ink-500">
          {isHost
            ? 'Press Play to start a random queue, or search your Plex library for a specific track.'
            : isHostUser
              ? 'Select Play here above to make this device active, then choose a track from Search.'
            : 'Waiting for the host to start music.'}
        </p>
        <AndroidAppDownload compact />
      </div>
    );
  }

  return (
    <div className="flex flex-col items-center gap-6 py-6 animate-fade-in sm:py-10">
      {/* Large album art */}
      <div className="relative">
        <div
          className={`absolute -inset-3 rounded-3xl bg-amber-500/10 blur-2xl transition-opacity duration-700 ${
            isPlaying ? 'opacity-100' : 'opacity-40'
          }`}
        />
        <CoverArt
          coverArt={current.coverArt}
          size={600}
          className="relative h-56 w-56 shrink-0 shadow-2xl shadow-black/50 sm:h-72 sm:w-72"
          rounded="rounded-2xl"
        />
        {isPlaying && (
          <div className="absolute bottom-3 right-3 flex items-end gap-0.5 rounded-lg bg-ink-950/80 px-2 py-1.5 backdrop-blur">
            {[4, 7, 10, 7, 4].map((h, i) => (
              <span
                key={i}
                className="w-1 rounded-full bg-amber-400"
                style={{
                  height: `${h}px`,
                  animation: `eq-bar 0.8s ease-in-out ${i * 0.12}s infinite alternate`,
                }}
              />
            ))}
          </div>
        )}
      </div>

      {/* Track info */}
      <div className="text-center">
        <h2 className="text-xl font-bold text-white sm:text-2xl">{current.title}</h2>
        <button onClick={() => onSearch(current.artist)} className="mt-1 text-sm text-ink-400 transition hover:text-amber-300 sm:text-base">{current.artist}</button>
        {current.album && (
          <button onClick={() => onSearch(current.album)} className="mt-0.5 block text-xs text-ink-500 transition hover:text-amber-300">{current.album}</button>
        )}
        <p className="mt-2 text-sm font-medium text-amber-400">
          Plex rating · {plexRating === null ? 'Not rated yet' : `${plexRating.toFixed(1)} / 10`}
        </p>
        <button onClick={() => void openDiscovery()} className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-ink-400 transition hover:text-amber-300"><Sparkles className="h-3.5 w-3.5" /> Artist discovery</button>
      </div>

      {/* Progress bar (read-only for guests) */}
      <div className="flex w-full max-w-md items-center gap-3">
        <span className="w-10 text-right text-xs tabular-nums text-ink-500">
          {formatTime(currentTime)}
        </span>
        <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-ink-700">
          <div
            className="absolute left-0 top-0 h-full rounded-full bg-amber-500 transition-all duration-200"
            style={{ width: `${pct}%` }}
          />
        </div>
        <span className="w-10 text-xs tabular-nums text-ink-500">
          {formatTime(total)}
        </span>
      </div>

      {/* Voting */}
      <div className="flex items-center gap-6">
        <button
          onClick={thumbsDown}
          title="Thumbs down"
          className="flex items-center gap-2 rounded-full border border-ink-700 px-5 py-3 text-ink-400 transition hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-400"
        >
          <ThumbsDown className="h-5 w-5" />
        </button>

        {!isHost && (
          <div className="flex items-center gap-1.5 text-ink-500">
            <Volume2 className="h-4 w-4" />
            <span className="text-xs">Playing on host</span>
          </div>
        )}

        <button
          onClick={thumbsUp}
          title="Thumbs up"
          className="flex items-center gap-2 rounded-full border border-ink-700 px-5 py-3 text-ink-400 transition hover:border-emerald-500/40 hover:bg-emerald-500/10 hover:text-emerald-400"
        >
          <ThumbsUp className="h-5 w-5" />
        </button>
      </div>

      {isHost && (
        <button
          onClick={queueSimilar}
          className="flex items-center gap-2 rounded-full border border-ink-700 px-4 py-2 text-xs font-medium text-ink-300 transition hover:border-amber-500/40 hover:bg-amber-500/10 hover:text-amber-300"
        >
          <WandSparkles className="h-4 w-4" /> Queue Track Radio
        </button>
      )}

      <AndroidAppDownload />
      {(discoveryLoading || discovery || discoveryError) && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 backdrop-blur-sm sm:items-center" role="dialog" aria-modal="true" aria-label="Artist discovery"><div className="flex max-h-[min(46rem,calc(100dvh-2rem))] w-full max-w-2xl flex-col rounded-3xl border border-ink-700 bg-ink-900 p-6 shadow-2xl"><div className="flex shrink-0 items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-amber-400">From Plex</p><h3 className="mt-1 text-xl font-black text-white">Artist discovery</h3></div><button onClick={() => { setDiscovery(null); setDiscoveryError(null); }} className="rounded-full p-2 text-ink-400 hover:bg-ink-800 hover:text-white" aria-label="Close artist discovery"><X className="h-5 w-5" /></button></div>{discoveryLoading ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin text-amber-400" /></div> : discoveryError ? <p className="py-8 text-sm text-red-300">{discoveryError}</p> : discovery && <div className="mt-5 min-h-0 overflow-y-auto pr-2"><h4 className="text-2xl font-black text-white">{discovery.name}</h4>{discovery.genres.length > 0 && <p className="mt-2 text-sm font-semibold text-amber-300">{discovery.genres.join(' · ')}</p>}{discovery.album.name && <section className="mt-5 rounded-2xl border border-ink-700 bg-ink-850 p-4"><p className="text-xs font-black uppercase tracking-[.16em] text-ink-500">Album context</p><p className="mt-1 font-bold text-white">{discovery.album.name}{discovery.album.year ? ` (${discovery.album.year})` : ''}</p>{discovery.album.summary && <p className="mt-2 text-sm leading-6 text-ink-300">{discovery.album.summary}</p>}</section>}{discovery.bio ? <p className="mt-5 whitespace-pre-line text-justify text-sm leading-7 text-ink-300 sm:text-base">{discovery.bio}</p> : <p className="mt-5 text-sm text-ink-500">Plex has no biography for this artist.</p>}{discovery.similarArtists.length > 0 && <section className="mt-7 border-t border-ink-700 pt-5"><h5 className="text-sm font-black uppercase tracking-[.16em] text-ink-400">Similar artists</h5><div className="mt-3 flex flex-wrap gap-2">{discovery.similarArtists.map((artist) => <span key={artist} className="rounded-full border border-ink-600 bg-ink-800 px-3 py-1.5 text-sm font-medium text-ink-200">{artist}</span>)}</div></section>}</div>}</div></div>}
    </div>
  );
}
